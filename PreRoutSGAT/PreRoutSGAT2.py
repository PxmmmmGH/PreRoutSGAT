# @Time :4/24/2024 11:14 AM
# @Author :Pxmmmm
# @File :PreRoutSGAT2.py
# @Version:  0.1
# @Conten:
...
class SignalProp(torch.nn.Module):
    ...
    def node_skip_level_o(self, nodes):
        ...
        # return {'new_nf': torch.cat([nodes.data['n_ats'], nodes.data['n_slew_log'], nodes.data['n_net_delays_log']], dim=1)}
...
class PreRoutSGAT2(torch.nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.nc3 = NetConv(self.nc_out * 2 + self.in_nf, 2, self.nc_out + self.nf_nd)
        self.cc = CellConv(self.in_nf, self.nc_out + self.nf_nd)
    ...
    def forward(self, g, ts, groundtruth=False, epoch=999999):
        ...
        net_delays = self.nc4(g, ts, torch.cat([nf0, x3[:, :self.nf_nd]], dim=1))

        nf1 = torch.cat([nf0, x3[:, self.nf_nd:]], dim=1)
