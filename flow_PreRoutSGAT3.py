# @Time :4/24/2024 9:23 PM
# @Author :Pxmmmm
# @File :flow_PreRoutSGAT3.py
# @Version:  0.1
# @Conten: model 3
...
from data_manager.data_preprocessing2 import data_train, data_test
from PreRoutSGAT.PreRoutSGAT3 import PreRoutSGAT3
    ...
    with torch.no_grad():
        def test_dict(data, istrain):
            ...
            for k, (g, ts) in data.items():
                ...
                time_s = time.time()
                net_delay, cell_delay, at, slew, rat = model(g, ts, groundtruth=groundtruth_test)  # nf
                torch.cuda.synchronize()
                time_t = time.time()
                ...
                r2_score_rat = r2_score(rat.cpu().numpy().reshape(-1), truth_rat.cpu().numpy().reshape(-1))
                ...
                print(
                    '{:.<12}: time {:.4f} r2_netdelay {:.4f}, r2_celldelay {:.4f}, r2_at {:.4f} r2_slew {:.4f} r2_rat {:.4f} slack_f {:.4f}, slack_unf {:.4f}, slack_ep_f {:.4f}, slack_ep_unf {:.4f}'.format(
                        k,
            ...
            if (istrain == False):
                ...
                print(
                    '-----------------test avg----------------- \nr2: netdelay {:1.4f}, celldelay {:1.4f}, at {:1.4f}, slew {:1.4f}, rat {:1.4f}, slack_f {:1.4f}, slack_unf {:1.4f}, slack_ep_f {:1.4f}, slack_ep_unf {:1.4f},\n'.format(
                        ...
                        test_r2_rat_batch_sum / len(data_test),
                ...
                train_r2_rat_list.append(train_r2_rat_batch_sum / len(data_train))
...
def train(model):
    for e in range(epoch):
        model.train()
        train_loss_tot_net_delays, train_loss_tot_cell_delays, train_loss_tot_at, train_loss_tot_slew, train_loss_tot_rat = 0, 0, 0, 0, 0
        optimizer.zero_grad()

        for k, (g, ts) in random.sample(data_train.items(), batch_size):
            pred_net_delays, pred_cell_delays, pred_at, pred_slew, pred_rat = model(g, ts, groundtruth=groundtruth,
                                                                                    epoch=999999)
            ...
            loss_rat = F.mse_loss(pred_rat, g.ndata['n_rats'])
            ...
            train_loss_tot_rat += loss_rat.item()

            (
                    loss_net_delays * lambda1 + loss_cell_delays * lambda2 + loss_at * lambda3 + loss_slew + loss_rat).backward()
        ...
        if e % 50 == 49:
            with torch.no_grad():
                model.eval()
                test_loss_tot_net_delays, test_loss_tot_cell_delays, test_loss_tot_at, test_loss_tot_slew, test_loss_tot_rat = 0, 0, 0, 0, 0

                for k, (g, ts) in data_test.items():
                    pred_net_delays, pred_cell_delays, pred_at, pred_slew, pred_rat = model(g, ts,
                    ...
                    test_loss_tot_rat += F.mse_loss(pred_rat, g.ndata['n_rats']).item()

                print(
                    'Epoch={}, net delay {:.6f}/{:.6f}, cell delay {:.6f}/{:.6f}, at {:.6f}/{:.6f}, slew {:.6f}/{:.6f}, rat {:.6f}/{:.6f}'.format(
                        e,
                        ...
                        train_loss_tot_rat / batch_size,
                        test_loss_tot_rat / len(data_test)
                    ))
                ...
                train_loss_rat_list.append(train_loss_tot_rat / batch_size)
                eval_loss_rat_list.append(test_loss_tot_rat / len(data_test))
...
load_param_pretrain_path = False#11999
load_param_pretrain_checkpoint = ""

load_path = 1199  # False
checkpoint = "model3"
...
if loop_gt0_result:
    ...
    report_r2(checkpoint_gt0, isSave=True, isRat=True)
    ...
    if checkpoint:
        ...
        test(model)
        train(model)
        report_loss(checkpoint, isSave=True, isRat=True)
        report_r2(checkpoint, isSave=True, isRat=True)
